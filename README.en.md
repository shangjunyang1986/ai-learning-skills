...
Don't want to remember four names? One front door, **[`/learn`](skills/learn)**, is enough — hand it any source (a GitHub link, an arXiv paper, a book, a topic) and it **auto-detects the type** and routes to the right skill above (asking one question first when it's ambiguous). (It can also aggregate all your generated pages into an [offline library hub](samples/learn-hub/index.html) — an optional extra, only worth it once you've built up many pages.)
...
- [x] Meta-layer entry: a [`learn`](skills/learn) router (auto-detect the source type and route). Plus an **optional** library hub (only worth it once you've built up many pages; example: [library](samples/learn-hub/index.html))
